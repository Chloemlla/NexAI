...
final _directionPattern = RegExp(
  r'^(?:graph|flowchart)\s+(TD|TB|LR|BT|RL)',
  caseSensitive: false,
);
final _nodeWithShape = RegExp(
  r'^(\w+)\s*(\[.*?\]|\(.*?\)|\(\(.*?\)\)|\{.*?\}|\[\/.*?\/\]|\[\[.*?\]\])',
);
...
MermaidGraph parseMermaid(String source) {
  ...
  for (final line in lines) {
    ...
    if (subMatch != null) {
      ...
      currentSubgraphLabel = subMatch.group(2)?.isNotEmpty == true
          ? subMatch.group(2)!
          : currentSubgraph;
    ...
    if (edgeMatch != null) {
      ...
      if (currentSubgraphNodes != null) {
        if (!currentSubgraphNodes.contains(fromId)) {
          currentSubgraphNodes.add(fromId);
        }
        if (!currentSubgraphNodes.contains(toId)) {
          currentSubgraphNodes.add(toId);
        }
      }
    ...
    if (bareId != null && bareId.group(1) != 'end') {
      ...
      if (currentSubgraphNodes != null &&
          !currentSubgraphNodes.contains(bareId.group(1)!)) {
...
void _parseChainedEdge(
  ...
  if (chain != null) {
    ...
    if (subgraphNodes != null && !subgraphNodes.contains(toId)) {
      subgraphNodes.add(toId);
    }
...
MermaidNode _parseNodeShape(String id, String raw) {
  if (raw.startsWith('((') && raw.endsWith('))')) {
    return MermaidNode(
      id: id,
      label: raw.substring(2, raw.length - 2),
  ...
  if (raw.startsWith('[[') && raw.endsWith(']]')) {
    return MermaidNode(
      id: id,
      label: raw.substring(2, raw.length - 2),
      shape: MermaidNodeShape.hexagon,
    );
  }
  if (raw.startsWith('[/') && raw.endsWith('/]')) {
    return MermaidNode(
  ...
  // Default [...]
  final label = raw.startsWith('[') && raw.endsWith(']')
