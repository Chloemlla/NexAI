...
public sealed class JsonConversationStore : IConversationStore
{
    ...
    {
        get { lock (_gate) { return _conversations.Select(c => c.Clone()).ToList(); } }
    }

    public string? CurrentConversationId
    {
        get { lock (_gate) { return _currentConversationId; } }
    }

    public Conversation? CurrentConversation
    {
        get { lock (_gate) { return FindCurrentUnlocked()?.Clone(); } }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            ...
            if (!string.IsNullOrWhiteSpace(currentConversationId) &&
                _conversations.Any(c => c.Id == currentConversationId))
            {
    ...
    private static Conversation Normalize(Conversation conversation)
    {
        ...
        if (string.IsNullOrWhiteSpace(clone.Id)) clone.Id = Guid.NewGuid().ToString("D");
        if (string.IsNullOrWhiteSpace(clone.Title)) clone.Title = "New chat";
        if (clone.CreatedAt == default) clone.CreatedAt = DateTime.UtcNow;
        if (clone.UpdatedAt == default) clone.UpdatedAt = clone.CreatedAt;
        clone.Messages ??= [];
